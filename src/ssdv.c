...
const PROGMEM uint8_t std_dht11[] = {
...
static void *stblcpy(ssdv_t *s, const void *src, size_t n)
{
	...
	r = memcpy_P(&s->stbls[s->stbl_len], src, n);
	s->stbl_len += n;
	return(r);
}
...
char ssdv_enc_init(ssdv_t *s, uint8_t type, char *callsign, uint8_t image_id, int8_t quality)
{
	...
	s->ddht[0][0] = dtblcpy(s, std_dht00, 29);
	s->ddht[0][1] = dtblcpy(s, std_dht01, 29);
	s->ddht[1][0] = dtblcpy(s, std_dht10, 179);
	s->ddht[1][1] = dtblcpy(s, std_dht11, 179);
...
char ssdv_dec_init(ssdv_t *s)
{
	...
	s->ddht[0][0] = stblcpy(s, std_dht00, 29);
  s->ddht[0][1] = stblcpy(s, std_dht01, 29);
  s->ddht[1][0] = stblcpy(s, std_dht10, 179);
  s->ddht[1][1] = stblcpy(s, std_dht11, 179);
